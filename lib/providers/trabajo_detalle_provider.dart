...
final trabajoDetalleProvider =
    StateNotifierProvider<TrabajoDetalleNotifier, AsyncValue<TrabajoDetalle?>>(
        (ref) {
  return TrabajoDetalleNotifier();
});

/// Provider para obtener lista de trabajos con detalles
final trabajosDetalleProvider = StateNotifierProvider<TrabajosDetalleNotifier,
    AsyncValue<List<TrabajoDetalle>>>((ref) {
  return TrabajosDetalleNotifier();
});

/// Notifier para manejar el estado de un trabajo específico con detalles
class TrabajoDetalleNotifier
    extends StateNotifier<AsyncValue<TrabajoDetalle?>> {
  ...
  Future<void> loadTrabajoDetalle(int trabajoId) async {
    ...
    try {
      final trabajoDetalle =
          await TrabajoDetalleService.getTrabajoDetalle(trabajoId);
...
class TrabajosDetalleNotifier
    extends StateNotifier<AsyncValue<List<TrabajoDetalle>>> {
  ...
  Future<void> loadTrabajosDetalle() async {
    ...
    try {
      final trabajosDetalle =
          await TrabajoDetalleService.getTrabajosDetalleOptimizado();
  ...
  List<TrabajoDetalle> getTrabajosByTipo(String tipo) {
    return state.when(
      data: (trabajos) => trabajos
          .where((t) => t.tipo.toLowerCase().contains(tipo.toLowerCase()))
          .toList(),
      loading: () => [],
      error: (_, __) => [],
    );
  ...
  List<TrabajoDetalle> getTrabajosByCultivo(String cultivo) {
    return state.when(
      data: (trabajos) => trabajos
          .where((t) => t.cultivo.toLowerCase().contains(cultivo.toLowerCase()))
          .toList(),
      loading: () => [],
      error: (_, __) => [],
    );
    ...
    return state.when(
      data: (trabajos) => trabajos
          .where((t) =>
              t.campo?.nombre
                  .toLowerCase()
                  .contains(campoNombre.toLowerCase()) ??
              false)
          .toList(),
      loading: () => [],
      error: (_, __) => [],
    ...
    return state.when(
      data: (trabajos) {
        ...
        final totalHectareas =
            trabajos.fold(0.0, (sum, t) => sum + t.totalHectareasPersonal);
        final totalPersonal =
            trabajos.fold(0, (sum, t) => sum + t.totalPersonal);
        final totalMaquinas =
            trabajos.fold(0, (sum, t) => sum + t.totalMaquinas);
